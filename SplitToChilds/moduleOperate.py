...
import onnx
# from onnx.onnx_pb import NodePro/to
...
class GraphNode():
    def __init__(self, node: NodeProto, TotalParams: List[str] = [], index: int = -1):
        ...
        # {"data": [{"type": str, "name": str, "shape": list, "cost": float}], "cost": float}
        self.input_info = {}
...
class ModelAnalyzer():
    ...
    def RuntimeAnalyze(self):
        ...
        if len(cache) < 1 or "data" not in cache or len(cache["data"]) != len(self.nodes):
            ...
            cache["create-time"] = time.strftime(
                "%Y-%m-%d %H:%M:%S", time.localtime())
            cache["data"] = {}
            tmp_onnx_path, tmp_param_path = Config.TempChildModelSavePathName(
                self.modelName)
            for idx, node in enumerate(self.nodes):
                info = self.ExtractModelByNode(
                    self.onnxPath, tmp_onnx_path, tmp_param_path, node, node, print_error=False)
        ...
        if len(self.nodes[0].input_info) < 1:     # empty-dict
            info = ModelAnalyzer.CreateParamsInfo(
                self.onnxPath, Config.ModelParamsSavePathName(self.modelName))
    ...
    def SplitAndStoreChilds(self, childs: List[GraphNode]) -> dict:
        ...
        total_param = {}
        childs = sorted(
            [child for child in childs if self.EnableStart(child)], key=lambda x: x.idx)
        ...
        for child_idx in range(len(childs)):
            ...
            print("{}-{} ==|>".format(self.modelName, child_idx),
            ...
            child_onnx_path, child_params_path = Config.ChildModelSavePathName(
                self.modelName, child_idx)
            info = self.ExtractModelByNode(
                self.onnxPath, child_onnx_path, child_params_path, start_node, end_node)
    ...
    def CreateParamsInfo(onnx_path: str, params_path: str, default_batch=15) -> Dict[str, dict]:
        ...
        params_dict = {"input": {"data": [], "cost": 0}, "output": {
            "data": [], "cost": 0}, "model_path": onnx_path}

        weight_params = set([v.name for v in model.graph.initializer])

        '''
        model.graph.input/output
        ...
        for k, tennsors in {"input": model.graph.input, "output": model.graph.output}.items():
            for m in tennsors:
                ...
                param["type"] = OnnxType.GetElemType(
                    m.type.tensor_type.elem_type)
                param["name"] = str(m.name)
                shape_list = []
                mul_value = 1
                for v in m.type.tensor_type.shape.dim:
                    shape_list.append(v.dim_value if isinstance(
                        v.dim_value, int) and v.dim_value > 0 else -1)
                    mul_value *= v.dim_value if isinstance(
                        v.dim_value, int) and v.dim_value > 0 else default_batch
            ...
            params_dict[k]["cost"] = params_dict[k]["cost"] / \
                (1024*1024)                       # MB

            os.makedirs(os.path.dirname(
                os.path.abspath(params_path)), exist_ok=True)
